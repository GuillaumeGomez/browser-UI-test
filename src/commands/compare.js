...
function parseCompareElementsAttributeInner(parser, assertFalse) {
    ...
    if (tuple.length < 3 || tuple.length > 4) {
        ...
        if (tuple[3].kind !== 'string') {
            ...
            return {
                'error': `Unknown operator \`${tuple[3].getRaw()}\`. Expected one of [` +
...
if (value1 === null) {
    throw attr + " (" + e1.getAttribute(attr) + ") from \`${selector1}\` isn't a number so \
comparison cannot be performed";
}
let value2 = browserUiTestHelpers.extractFloat(e2.getAttribute(attr));
if (value2 === null) {
    throw attr + " (" + e2.getAttribute(attr) + ") from \`${selector2}\` isn't a number so \
comparison cannot be performed";
}
${insertAfter}if (e1.getAttribute(attr) ${matchings[operator]} e2.getAttribute(attr)) {
